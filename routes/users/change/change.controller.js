  ...
  jwt.verify(token, process.env.JWT_SECRET, (err, decoded) => {
    ...
    Account.findOneAndUpdate({ user_id: decoded.user_id }, { user_name: new_username }, { new: true })
      .then(account => {
        ...
        const newToken = jwt.sign({ user_id: decoded.user_id, user_name: new_username, auth_provider: decoded.auth_provider }, process.env.JWT_SECRET, { expiresIn: '24h' });
        return res.status(200).json({ message: '사용자 이름이 성공적으로 변경되었습니다.', token: newToken });
      })
      .catch(err => {
        if (!res.headersSent)
